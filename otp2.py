...
async def main():
    ...
    async def handler(event):
        ...
        # Mesajı hazırla - MEDYA VARSA EMOJİ İLE DEĞİŞTİR
        son_mesaj = text
        if kodlar:
            son_mesaj += f"\n\n🔑 Kodlar:\n" + "\n".join(kodlar)
        
        # Eğer medya varsa, onu emoji ile değiştir
        if msg.media:
            # Rastgele emoji seç
            emoji = random.choice(EMOJILER)
            
            # Emoji ile mesaj gönder (medya olmadan)
            emoji_mesaj = f"{emoji} {son_mesaj}" if son_mesaj else emoji
            await client.send_message(sizin, emoji_mesaj)
            print(f"📨 Medya emoji ile değiştirildi: {emoji} - Mesaj ID: {msg.id}")
        else:
            # Medya yoksa normal mesaj gönder
