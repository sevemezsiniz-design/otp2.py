...
async def mesaj_ozetini_al(msg):
    """Mesajın özetini çıkar (medya olsa bile)"""
    ozet = []
    
    # Mesaj metni varsa ekle
    if msg.message:
        ozet.append(msg.message)
    ...
    # Özet metni oluştur
    son_mesaj = "\n".join(ozet) if ozet else "📨 Medya mesajı"
...
async def main():
    ...
    async def handler(event):
        ...
        # Mesaj özetini ve kodları al
        mesaj_ozeti, kod_sayisi = await mesaj_ozetini_al(msg)
        
        # Mesajı gönder
        try:
            if mesaj_ozeti.strip():
                await client.send_message(sizin, mesaj_ozeti)
                logger.info(f"📨 İletildi: {msg.id} - Kodlar: {kod_sayisi} adet")
            else:
    ...
    logger.info("✨ Bot çalışıyor... (Medya özeti + buton kodları iletiliyor)")
